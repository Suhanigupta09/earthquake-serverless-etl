...
def lambda_handler(event, context):
    try:
        logger.info("JSON processor started")
        ...
        records = extract_json_records(content)
        ...
        logger.info("JSON Audit Summary: %s", json.dumps(audit_summary))
        ...
        logger.exception("JSON processor failed")

        return {
            "statusCode": 500,
            "body": json.dumps({
                "message": "JSON ETL failed",
                "error": str(error)
            })
        }